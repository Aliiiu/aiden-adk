...
Use this for: agent discovery dashboards, browsing interfaces, market overviews, filtered searches, and REQUIRED lookups before using other tools.`,
	schema: z.object({
		sort: z
			...
			.optional()
			.describe("Sort field (default: marketCap)"),
		order: z
			.enum(["asc", "desc"])
			.optional()
		...
		chainId: z
			.number()
			.optional()
			.describe("Filter by blockchain chain ID (e.g., 252)")
			.default(252),
		page: z
			.number()
			.optional()
		...
		limit: z
			.number()
			.optional()
			.describe("Agents per page, max 100 (default: 50)"),
	}),
	fn: async ({
		sort,
		order,
...
Use this for: leaderboards, trending agent widgets, top performer displays, quick market snapshots.`,
	schema: z.object({
		sort: z
			...
			.optional()
			.describe("Ranking metric (default: mcap)"),
		limit: z
			.number()
			.min(1)
			.max(100)
			.optional()
			.describe("Number of agents to return, 1-100 (default: 10)"),
	}),
...
Use this for: profile pages, agent cards, detailed views, social bot configuration.`,
	schema: z.object({
		address: z
			...
			.optional()
			.describe(
				"Token contract address of the agent (MUST be obtained from get_all_agents first)",
			),
		ticker: z
			.string()
			.optional()
			.describe(
				"Agent ticker symbol (may return multiple agents, MUST be obtained from get_all_agents first)",
			),
	}),
...
Use this for: price displays, market analytics, performance tracking, trading dashboards.`,
	schema: z.object({
		address: z
			...
			.optional()
			.describe(
				"Token contract address (MUST be obtained from get_all_agents first)",
			),
		ticker: z
			.string()
			.optional()
			.describe(
				"Agent ticker (returns array of basic stats, MUST be obtained from get_all_agents first)",
			),
		extendedStats: z
			.boolean()
			.optional()
			.describe("Include ATH/ATL, timelines, 24h trading data (address only)"),
	}),
...
Use this for: activity timelines, audit trails, event monitoring, transaction tracking.`,
	schema: z.object({
		agentTokenContract: z
			.string()
			.describe(
				"Token contract address of the agent (REQUIRED - MUST be obtained from get_all_agents first)",
			),
		page: z
			.number()
			.optional()
		...
		limit: z
			.number()
			.optional()
			.describe("Logs per page, max 100 (default: 10)"),
	}),
...
Use this for: portfolio displays, wallet analysis, holdings tracking, valuation calculations.`,
	schema: z.object({
		address: z
			.string()
			.describe(
				"Wallet address to query (verify using get_all_agents if needed)",
			),
		chainId: z
			.string()
			.optional()
			.describe("Chain ID of the agent")
			.default("252"),
	}),
